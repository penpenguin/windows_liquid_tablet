...
EXPECTED_IDD_INF_FILENAME = "windows_liquid_tablet_idd.inf"
EXPECTED_IDD_CATALOG_FILENAME = "windows_liquid_tablet_idd.cat"

IDD_METADATA_PATH_FIELDS = (
    "Driver package path",
    "INF path",
    "Catalog file",
    "Native preflight evidence path",
...
IDD_EVIDENCE_PATH_METADATA_FIELDS = (
    "Native preflight evidence path",
    "Runtime evidence path",
)
...
def duplicate_metadata_path_values(metadata: dict[str, str]) -> list[tuple[str, str, str]]:
    ...
    for field in IDD_EVIDENCE_PATH_METADATA_FIELDS:
...
def validate_idd_verification_evidence_text(text: str) -> list[str]:
    failures: list[str] = []
    ...
    for field in IDD_METADATA_PATH_FIELDS:
        path_value = metadata.get(field, "")
        if path_value != "":
    ...
    if inf_path != "" and not normalize_metadata_value(inf_path).endswith(EXPECTED_IDD_INF_FILENAME):
        failures.append("INF path must end with windows_liquid_tablet_idd.inf")

    catalog_file = metadata.get("Catalog file", "")
    if catalog_file != "" and not normalize_metadata_value(catalog_file).endswith(EXPECTED_IDD_CATALOG_FILENAME):
        failures.append("Catalog file must end with windows_liquid_tablet_idd.cat")
...
def read_idd_verification_evidence_text(evidence_path: Path) -> tuple[str | None, list[str]]:
    if evidence_path.is_symlink():
        return None, [f"IDD verification evidence path must not be a symbolic link: {evidence_path}"]
    if path_has_symlink_parent(evidence_path):
        return None, [
            f"IDD verification evidence path parent directories must not be symbolic links: {evidence_path}"
        ]
    if evidence_path.exists() and not evidence_path.is_file():
        return None, [f"IDD verification evidence path must be a file: {evidence_path}"]
    try:
        return evidence_path.read_text(encoding="utf-8-sig"), []
    except FileNotFoundError:
        return None, [f"IDD verification evidence file is missing: {evidence_path}"]
    except UnicodeDecodeError:
        return None, [f"IDD verification evidence is not valid UTF-8: {evidence_path}"]


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate completed Windows Liquid Tablet IDD verification evidence."
    )
    parser.add_argument("evidence_path", type=Path)
    args = parser.parse_args(argv)

    text, failures = read_idd_verification_evidence_text(args.evidence_path)
    if text is not None:
        failures.extend(validate_idd_verification_evidence_text(text))
    if failures:
        for failure in failures:
            print(failure, file=sys.stderr)
        return 1

    print("IDD verification evidence validates.")
    return 0
