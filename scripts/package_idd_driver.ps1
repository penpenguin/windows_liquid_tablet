param(
    [string]$DriverBinary = "windows\idd_driver\bin\x64\Debug\WindowsLiquidTabletIdd.dll",
    [string]$InfPath = "",
    [string]$OutputDir = "artifacts\idd_driver",
...
function Assert-PackageToolPathSafe {
    ...
    if (Test-PathIsSymlink $ResolvedToolPath) {
        throw "IDD WDK package tool path must not be a symbolic link: $ResolvedToolPath"
    }
    if (Test-PathHasSymlinkParent $ResolvedToolPath) {
        throw "IDD WDK package tool path parent directories must not be symbolic links: $ResolvedToolPath"
    }
    if (-not (Test-Path -LiteralPath $ResolvedToolPath -PathType Leaf)) {
        throw "IDD WDK package tool path must be a file: $ResolvedToolPath"
    }
}
...
function Assert-PackageOutputFilePathSafe {
    ...
    $symlinkFailureByName = @{
        "staged INF" = "IDD driver package staged INF output path must not be a symbolic link"
        "staged driver binary" = "IDD driver package staged driver binary output path must not be a symbolic link"
        "catalog" = "IDD driver package catalog output path must not be a symbolic link"
    }
    $symlinkParentFailureByName = @{
        "staged INF" = "IDD driver package staged INF output path parent directories must not be symbolic links"
        "staged driver binary" = "IDD driver package staged driver binary output path parent directories must not be symbolic links"
        "catalog" = "IDD driver package catalog output path parent directories must not be symbolic links"
    }
...
if (Test-PathIsSymlink $resolvedOutput) {
    throw "IDD driver package output directory must not be a symbolic link: $resolvedOutput"
}
if (Test-PathHasSymlinkParent $resolvedOutput) {
    throw "IDD driver package output directory parent directories must not be symbolic links: $resolvedOutput"
}
if ((Test-Path -LiteralPath $resolvedOutput) -and -not (Test-Path -LiteralPath $resolvedOutput -PathType Container)) {
    throw "IDD driver package output path must be a directory: $resolvedOutput"
}
...
if ($resolvedOutputParent -ne "" -and (Test-Path -LiteralPath $resolvedOutputParent) -and -not (Test-Path -LiteralPath $resolvedOutputParent -PathType Container)) {
    throw "IDD driver package output parent path must be a directory: $resolvedOutputParent"
}

if (Test-PathIsSymlink $resolvedInf) {
    throw "IDD driver package INF path must not be a symbolic link: $resolvedInf"
}
if (Test-PathHasSymlinkParent $resolvedInf) {
    throw "IDD driver package INF path parent directories must not be symbolic links: $resolvedInf"
}
if ((Test-Path -LiteralPath $resolvedInf) -and -not (Test-Path -LiteralPath $resolvedInf -PathType Leaf)) {
    throw "IDD driver package INF path must be a file: $resolvedInf"
}
...
if (Test-PathIsSymlink $resolvedDriver) {
    throw "IDD driver package binary path must not be a symbolic link: $resolvedDriver"
}
if (Test-PathHasSymlinkParent $resolvedDriver) {
    throw "IDD driver package binary path parent directories must not be symbolic links: $resolvedDriver"
}
if ((Test-Path -LiteralPath $resolvedDriver) -and -not (Test-Path -LiteralPath $resolvedDriver -PathType Leaf)) {
    throw "IDD driver package binary path must be a file: $resolvedDriver"
}
if (-not (Test-Path -LiteralPath $resolvedDriver -PathType Leaf)) {
    throw "Driver binary not found: $resolvedDriver. Build the WDK built UMDF driver before packaging."
}
...
$normalizedInf = (Get-Content -LiteralPath $resolvedInf) -replace "WindowsLiquidTabletIdd.dll", "windowsliquidtabletidd.dll"
...
if ($normalizedInf -match [regex]::Escape($unresolvedUmdfVersion)) {
    throw "IDD driver package INF must be the stamped build output; found unresolved $unresolvedUmdfVersion in $resolvedInf"
}
...
if (-not (Test-Path -LiteralPath $catalogPath -PathType Leaf)) {
    throw "IDD driver package catalog path must be a file: $catalogPath"
}
...
Write-Host "Packaged IDD driver at $resolvedOutput"
