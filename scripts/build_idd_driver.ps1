...
function Assert-MSBuildToolPathSafe {
    ...
    if (Test-PathIsSymlink $ResolvedMSBuildPath) {
        throw "IDD WDK MSBuild tool path must not be a symbolic link: $ResolvedMSBuildPath"
    }
    if (Test-PathHasSymlinkParent $ResolvedMSBuildPath) {
        throw "IDD WDK MSBuild tool path parent directories must not be symbolic links: $ResolvedMSBuildPath"
    }
    if (-not (Test-Path -LiteralPath $ResolvedMSBuildPath -PathType Leaf)) {
        throw "IDD WDK MSBuild tool path must be a file: $ResolvedMSBuildPath"
    }
}
...
if (Test-PathIsSymlink $resolvedProject) {
    throw "IDD WDK project path must not be a symbolic link: $resolvedProject"
}
if (Test-PathHasSymlinkParent $resolvedProject) {
    throw "IDD WDK project path parent directories must not be symbolic links: $resolvedProject"
}
if ((Test-Path -LiteralPath $resolvedProject) -and -not (Test-Path -LiteralPath $resolvedProject -PathType Leaf)) {
    throw "IDD WDK project path must be a file: $resolvedProject"
}
...
$builtDriver = Join-Path $repoRoot "windows\idd_driver\bin\$Platform\$Configuration\WindowsLiquidTabletIdd.dll"
if ((Test-Path -LiteralPath $builtDriver) -and -not (Test-Path -LiteralPath $builtDriver -PathType Leaf)) {
    throw "Built IDD UMDF driver path must be a file: $builtDriver"
}
if (-not (Test-Path -LiteralPath $builtDriver -PathType Leaf)) {
    throw "Built UMDF driver was not found at expected path: $builtDriver"
}
...
Write-Host "Built IDD UMDF driver at $builtDriver"
