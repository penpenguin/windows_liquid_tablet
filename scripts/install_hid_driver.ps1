...
function Validate-InstanceId {
    ...
    if ($InstanceId -ne "WindowsLiquidTabletHidPen") {
        throw "InstanceId must be WindowsLiquidTabletHidPen: $InstanceId"
    }
}
...
function Assert-InstallToolPathSafe {
    ...
    if (Test-PathIsSymlink $ResolvedToolPath) {
        throw "Optional HID install tool path must not be a symbolic link: $ResolvedToolPath"
    }
    if (Test-PathHasSymlinkParent $ResolvedToolPath) {
        throw "Optional HID install tool path parent directories must not be symbolic links: $ResolvedToolPath"
    }
    if (-not (Test-Path -LiteralPath $ResolvedToolPath -PathType Leaf)) {
        throw "Optional HID install tool path must be a file: $ResolvedToolPath"
    }
}
...
if (Test-PathIsSymlink $resolvedInf) {
    throw "Optional HID driver INF path must not be a symbolic link: $resolvedInf"
}
if (Test-PathHasSymlinkParent $resolvedInf) {
    throw "Optional HID driver INF path parent directories must not be symbolic links: $resolvedInf"
}
if ((Test-Path -LiteralPath $resolvedInf) -and -not (Test-Path -LiteralPath $resolvedInf -PathType Leaf)) {
    throw "Optional HID driver INF path must be a file: $resolvedInf"
}
...
Write-Host "Installed optional HID development device for hardware ID $HardwareId"
