...
function Assert-UninstallToolPathSafe {
    ...
    if (Test-PathIsSymlink $ResolvedToolPath) {
        throw "IDD uninstall tool path must not be a symbolic link: $ResolvedToolPath"
    }
    if (Test-PathHasSymlinkParent $ResolvedToolPath) {
        throw "IDD uninstall tool path parent directories must not be symbolic links: $ResolvedToolPath"
    }
    if (-not (Test-Path -LiteralPath $ResolvedToolPath -PathType Leaf)) {
        throw "IDD uninstall tool path must be a file: $ResolvedToolPath"
    }
}
...
Write-Host "Removed IDD development device for hardware ID $HardwareId"
